...
internals.schema = Joi.object().keys({
    ...
    authByQueryKeysExistence: Joi.array().items(Joi.string().required())
});
...
internals.implementation = (server, options) => {
    ...
    const scheme = {
        authenticate: async (request, h) => {
            ...
            const queryKeys = settings.authByQueryKeysExistence;
            if (!authorization && !settings.allowQueryToken && Array.isArray(queryKeys)) {
                const keysExist = queryKeys.every(key => typeof request.query[key] !== 'undefined');
                if (keysExist) {
                    authorization = `${settings.tokenType} doesntmatter`;
                } else {
